...
<?php
if (isset($_POST['idPaciente']) && isset($_POST['idEstudio']) && isset($_POST['idProfesional']) && isset($_POST['nombre']) && isset($_POST['fecha'])) {
    ...
    // Muestro nombre servicio seleccionado
    echo "Estudio: $nombre<br>";

    // Muestro fecha seleccionada
    echo "Fecha: $fecha<br>";
}
?>
                                </div>
...
<?php 
    ...
    $query = "SELECT DATE_FORMAT(STR_TO_DATE(horario, '%H:%i'), '%H:%i') AS horario 
    FROM htd WHERE idEstudio = $idEstudio AND sobreturno = 0 AND DATE_FORMAT(STR_TO_DATE(horario, '%H:%i'), '%H:%i') 
    NOT IN (SELECT DATE_FORMAT(fechaHora,'%H:%i') FROM turnos WHERE idProfesional = $idProfesional AND DATE(fechaHora) = '$fecha');";
